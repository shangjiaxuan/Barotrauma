...
namespace Barotrauma{
	public class PrefabInstance
	{
		...
		public static PrefabInstance EmptyInstance = new PrefabInstance("".ToIdentifier(), "");
	}
	...
	public interface IImplementsInherit<T> where T : Prefab
	{
		...
		public IEnumerable<T> InheritHistory
		{
			get
			{
				IImplementsInherit<T> cur = this;
				while (!(cur.InheritParent?.IsEmpty ?? true))
				{
					if (cur.InheritParent.package.IsNullOrEmpty())
					{
						cur = cur.GetPrevious(cur.InheritParent.id) as IImplementsInherit<T>;
					}
					else
					{
						cur = FindByPrefabInstance(cur.InheritParent) as IImplementsInherit<T>;
					}
		...
		{
			bool result = true;
			if ((parent as IImplementsInherit<T>).InheritHistory.Any(p => ReferenceEquals(p, this as T)))
			{
		...
		public ContentXElement DoInherit(Action<ContentXElement, ContentXElement, ContentXElement> create_callback)
		{
			...
			{
				preprocessed.Push(last_elem.PreprocessInherit((it as IImplementsInherit<T>).originalElement.FromContent(it.ContentFile.Path), false));
		...
		public static PrefabCollection<T> Prefabs_inherit
		{
			get
			{
				var type = typeof(T);
				var collectionField = type.GetField("Prefabs", BindingFlags.Public | BindingFlags.Static);
				if (collectionField is null) { throw new InvalidOperationException($"Couldn't determine PrefabCollection for {type.Name}"); }
				return (PrefabCollection<T>)collectionField.GetValue(null) ?? throw new InvalidOperationException($"PrefabCollection for {type.Name} was null");
			}
		}

		public T FindByPrefabInstance(PrefabInstance instance)
		{
			Prefabs_inherit.TryGet(instance, out T res);
			return res;
		}
		public T GetPrevious(Identifier identifier)
		{
			T res;
			if (identifier != (this as T).Identifier)
			{
				if (Prefabs_inherit.Any(p => p.Identifier == identifier))
				{
					res = Prefabs_inherit[identifier];
				}
				else
				{
					res = null;
			...
			{
				if (Prefabs_inherit.AllPrefabs.Any(p => p.Key == identifier))
				{
					string best_effort_package_id;
					if ((this as T).ContentPackage.UgcId.TryUnwrap(out ContentPackageId id))
					{
						best_effort_package_id = id.ToString();
					}
					else {
						best_effort_package_id = (this as T).ContentPackage.Name;
					}
					res = Prefabs_inherit.AllPrefabs.Where(p => p.Key == identifier)
						.Single().Value
